...
Output: site/data/latest.json (+ site/data/history/YYYY-MM-DD.json after a full scan)
"""
...
# --------------------------------------------------------------------------- stage 1: Finviz
FINVIZ = ("https://finviz.com/screener.ashx?v=152&f=sh_opt_option,sh_price_u10,sh_short_o15"
          "&o=-shortinterestshare&c=1,2,6,30,31,42,43,44,52,53,54,57,58,59,63,64,65,66,67,68&r={r}")
COLS = ["t", "co", "mc", "sf", "sr", "pw", "pm", "pq", "s20", "s50", "s200", "hi", "lo", "rsi", "av", "rv", "p", "ch", "v", "earn"]

def num(s):
    if s is None or (isinstance(s, float) and math.isnan(s)) or s == "-": return float("nan")
    s = str(s).replace(",", "").replace("%", "")
    m = 1
    if s.endswith("K"): m, s = 1e3, s[:-1]
...
def fix_ticker(t):
    t = str(t).strip()
    if len(t) % 2 == 0 and t[: len(t)//2] == t[len(t)//2:]: return t[len(t)//2:]
    if len(t) > 2 and t[0] == t[1]: return t[1:]
    return t

def finviz_page(r):
    resp = requests.get(FINVIZ.format(r=r), headers=UA, timeout=30)
    resp.raise_for_status()
    html = resp.text
    total = int((re.search(r"#\d+ / (\d+) Total", html) or [None, "0"])[1])
    tables = pd.read_html(io.StringIO(html))
    df = next((t for t in tables if list(t.columns[:2]) == ["Ticker", "Company"]), None)
    if df is None: raise RuntimeError("Finviz screener table not found (blocked? layout change?)")
    df = df.iloc[:, :20]; df.columns = COLS
    return df, total

def stage1():
    df, total = finviz_page(1)
    frames = [df]
    for r in range(21, total + 1, 20):
        time.sleep(1.2)
        frames.append(finviz_page(r)[0])
    df = pd.concat(frames, ignore_index=True)
    df["t"] = df["t"].map(fix_ticker)
    df = df.drop_duplicates(subset="t").reset_index(drop=True)
    for c in COLS[2:19]: df[c] = df[c].map(num)
    df["earn"] = df["earn"].fillna("-").astype(str)
    ...
    top = [{k: (None if isinstance(x[k], float) and math.isnan(x[k]) else x[k]) for k in
            ("t", "co", "p", "sf", "sr", "rsi", "lo", "pw", "pm", "pq", "s20", "rv", "av", "mc", "earn", "s1")} for x in keep[: CFG["deep_n"]]]
...
def assemble(mode, universe, pass1, top_in, deep_out, full_asof):
    ...
    return dict(asof=datetime.now(timezone.utc).isoformat(timespec="seconds").replace("+00:00", "Z"),
